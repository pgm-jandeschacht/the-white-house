<?php
    ...
    $page_object = get_post_type_archive_link( 'president' );
?>

<section class="px-[25px] mb-[75px]">
    <div class="mx-[-10px] flex flex-col md:flex-row">
        <div class="min-h-[540px] w-full max-w-full md:max-w-half flex">
            <div class="rounded-[20px] md:rounded-tr-none overflow-hidden w-full relative">
                <?php the_post_thumbnail( 'full', ['class' => 'absolute bottom-0 left-0 object-cover w-full h-full float-left'] ); ?>
            </div>
        </div>
    
        <div class="md:max-w-half flex justify-center items-center w-full">
            <div class="max-w-[58.333%] mx-auto my-[20px] flex flex-col items-center">
                ...
                <h1 class="font-mercury font-variant capitalize text-[26px] md:text-[36px] lg:text-[44px] font-semibold tracking-tight mt-2 text-center leading-[108%]">
                    <?php the_title(); ?>
                </h1>

                <?php if (get_field('order_of_president')) : ?>
...
<section class="px-[35px]">
    <div class="mx-[-10px] flex">
        <div class="content">
            <div class="max-w-[16.666%] ml-[8.333%] px-[10px] float-right hidden lg:block">
                <?php if (is_active_sidebar('side_nav_presidents')) : ?>
                    <nav aria-label="Side Navigation" class="side-navigation relative before:content-[''] before:h-full before:w-[1px] before:bg-line before:absolute before:left-[-20px] before:top-0">
                        ...
                        <div class="mt-[30px]">
                            <?php dynamic_sidebar('side_nav_presidents'); ?>
                        </div>
                    </nav>
                <?php endif; ?>
            </div>
